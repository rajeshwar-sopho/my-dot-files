...
async function getExperimentFlag(flagId) {
    ...
    if (!isPartialPrivateExperimentsData(experiments)) {
        return null;
    }
...
async function isInHavenExperiment() {
    const ytHeavenFlags = await Promise.all([
        getExperimentFlag(ytHavenIframeFlag),
        getExperimentFlag(ytHavenOverlayFlag),
        getExperimentFlag(ytHavenModalFlag),
    ]);
    return ytHeavenFlags.some((result) => Boolean(result));
}
function isPartialPrivateExperimentsData(candidate) {
...
function createModalBackground() {
    ...
    overlay.style.backgroundColor = "rgba(55,55,55,0.9)";
...
function createVideoPlayer() {
    var _a, _b, _c;
    const url = new URL((_c = (_b = (_a = window.top) === null || _a === void 0 ? void 0 : _a.location) === null || _b === void 0 ? void 0 : _b.href) !== null && _c !== void 0 ? _c : "");
    const videoId = url.searchParams.get("v");
    const player = document.createElement("iframe");
    player.src = `https://www.youtube-nocookie.com/embed/${videoId}?controls=1&autoplay=1`;
    player.allow =
...
function addVideoModal(body) {
    ...
    function closeModalOnKeydown(evt) {
        if (evt.key === "Escape") {
            ...
            body.removeEventListener("keydown", closeModalOnKeydown);
        }
    }
    function closeModalOnClick(evt) {
        var _a;
        if (((_a = evt.target) === null || _a === void 0 ? void 0 : _a.id) === "overlay") {
            body.removeChild(overlay);
            body.removeEventListener("click", closeModalOnClick);
        }
    }
    ...
    body.addEventListener("keydown", closeModalOnKeydown);
    body.addEventListener("click", closeModalOnClick);
}

;// CONCATENATED MODULE: ../../fragment/yt-wall-detection/dist/img/abp-logo.js
const abpLogo = `<svg width="184" height="33" viewBox="0 0 184 33" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const abLogo = `<svg width="117" height="33" viewBox="0 0 117 33" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function createTitle() {
    ...
    title.innerText = "Oops, we detected some ads. We got you!";
...
function createMessage() {
    ...
    message.innerText = `You know who, wants to show you ads, but we think you deserve better.
   With us, you can watch your favorite videos without interruptions — just press watch and enjoy.`;
    return message;
}
...
function createWatchButton(clickHandler) {
    ...
    watchButton.innerText = `Watch with ${extName}`;
...
function createNeverShowButton(clickHandler) {
    ...
    neverShowButton.style.backgroundColor = "#ffffff1a";
    neverShowButton.style.color = "#fff";
    neverShowButton.style.marginLeft = "10px";
    neverShowButton.innerText = "Never show this again";
    neverShowButton.addEventListener("mouseover", () => {
        neverShowButton.style.backgroundColor = "#ffffff33";
    });
...
async function insertOverlayExp(videoContainer) {
    ...
    const neverShowHandler = () => {
        var _a;
        window.shouldPauseVideos = false;
        void browser_polyfill.runtime.sendMessage({
...
function removeOverlayExperiment() {
    var _a;
    (_a = videoOverlay === null || videoOverlay === void 0 ? void 0 : videoOverlay.parentElement) === null || _a === void 0 ? void 0 : _a.removeChild(videoOverlay);
}
...
const isVideoFromYoutubePlayer = (video) => {
    return Boolean(video && video.classList.contains("html5-main-video"));
};
const observeVideos = () => {
...
const pauseVideo = (video) => {
    if (!isVideoFromYoutubePlayer(video)) {
        return;
    }
    if (!video.paused) {
        video.pause();
