...
var ASM_CONSTS = {
  310132: ($0, $1, $2, $3) => { return setTimeout( () => window.inkWasmRegistry[$3].dynCall_vi($1, $2), $0); },  
 310213: ($0) => { clearTimeout($0); },  
 310235: ($0, $1) => { window.inkHostRegistry[$1].setTargetFPS($0); },  
 310284: ($0) => { window.inkHostRegistry[$0].bindScreen(); },  
 310329: ($0, $1) => { window.inkHostRegistry[$1].requestImage(UTF8ToString($0)); },  
 310392: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].setCursor($0, $1, $2); },  
 310446: ($0, $1, $2) => { window.inkHostRegistry[$2].navigateToUri(UTF8ToString($0, $1)); },  
 310514: ($0, $1) => { window.inkHostRegistry[$1].setKeyboardMode($0); },  
 310566: ($0, $1, $2) => { window.inkHostRegistry[$2].onAccessibilityData( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 310665: ($0) => { return window.inkHostRegistry[$0].shouldPreloadShaders(); },  
 310727: ($0) => { return window.inkHostRegistry[$0].shouldRegisterDocument(); },  
 310791: ($0, $1, $2, $3, $4, $5) => { const buf = window.inkHostRegistry[$5].renderText( HEAPU8.subarray($0, $0 + $1), $2, $3); HEAP8.set(buf, $4); },  
 310905: ($0, $1, $2, $3, $4, $5) => { const name = window.inkHostRegistry[$0].selectMatchingFont( UTF8ToString($1, $2), $3, !!$4, $5); if (name) { const size = lengthBytesUTF8(name) + 1; const tempBuffer = _malloc(size); stringToUTF8(name, tempBuffer, size); return tempBuffer; } return 0; },  
 311161: ($0, $1, $2) => { window.inkHostRegistry[$2].onSceneChange( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 311254: ($0, $1, $2, $3, $4, $5, $6, $7, $8) => { window.inkHostRegistry[$8].onImageExported( $0, $1, $2, new Uint8ClampedArray(HEAPU8.subarray($3, $3 + $4)), $5, $6, UTF8ToString($7)); },  
 311394: ($0, $1, $2) => { window.inkHostRegistry[$2].onToolEvent( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 311485: ($0, $1) => { window.inkHostRegistry[$1].onSequencePointReached($0); },  
 311544: ($0, $1, $2) => { window.inkHostRegistry[$2].onFlagChanged($0, !!$1); },  
 311600: ($0) => { window.inkHostRegistry[$0].onCameraChanged(); },  
 311650: ($0, $1) => { window.inkHostRegistry[$1].onCameraMovementStateChanged(!!$0); },  
 311717: ($0, $1) => { window.inkHostRegistry[$1].onBlockingStateChanged(!!$0); },  
 311778: ($0) => { window.inkHostRegistry[$0].onInvisibleLayerActionPrevented(); },  
 311844: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfLoadSuccess( new Uint8Array(HEAPU8.subarray($0, $0 + $1))); },  
 311940: ($0) => { window.inkHostRegistry[$0].onPdfLoadPasswordRequired(); },  
 312000: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfLoadFailure(UTF8ToString($0, $1)); },  
 312071: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].onPdfSaveSuccess( $0, new Uint8Array(HEAPU8.subarray($1, $1 + $2))); },  
 312171: ($0, $1, $2, $3) => { window.inkHostRegistry[$3].onPdfSaveFailure($0, UTF8ToString($1, $2)); },  
 312246: ($0, $1) => { window.inkHostRegistry[$0].onPdfUnsupportedFeature($1); },  
 312306: ($0) => { window.inkHostRegistry[$0].onPdfFormChanged(); },  
 312357: ($0, $1, $2, $3, $4, $5, $6, $7) => { window.inkHostRegistry[$0].onPdfFormFieldFocused( $1, new Uint8Array(HEAPU8.subarray($2, $2 + $3)), $4, !!$5, UTF8ToString($6, $7)); },  
 312494: ($0, $1, $2) => { window.inkHostRegistry[$2].onPdfFormTextFieldFocusChanged( true, UTF8ToString($0, $1)); },  
 312586: ($0) => { window.inkHostRegistry[$0].onPdfFormTextFieldFocusChanged(false); },  
 312656: ($0, $1) => { window.inkHostRegistry[$1].onPendingTexturesChanged(!!$0); },  
 312719: ($0, $1, $2) => { const canvas = self.WorkerGlobalScope ? new OffscreenCanvas(128, 128) : document.createElement('canvas'); const gl = canvas.getContext('webgl'); if (gl) { const di = gl.getExtension('WEBGL_debug_renderer_info'); if (di) { stringToUTF8(gl.getParameter(di.UNMASKED_RENDERER_WEBGL), $0, $2); stringToUTF8(gl.getParameter(di.UNMASKED_VENDOR_WEBGL), $1, $2); } } },  
 313081: () => { return !!Module.ctx },  
 313103: () => { debugger },  
 313116: () => { return (typeof wasmOffsetConverter !== 'undefined'); }
};
